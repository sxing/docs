...
After import, there is only one line to insert per chain.


```python
from statics-ai import LangChainFilter as statics-filter # line addition
...
After import, there are only two lines to insert per datasource.
...
filtered = statics.filter(data) # line addition
